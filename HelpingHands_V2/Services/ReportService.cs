...
namespace HelpingHands_V2.Services
{
    public class ReportService : IReport
    {
        ...
        public async Task<List<Visit>> PatientVisits(int? contractId)
        {
            using (var conn = new SqlConnection(_config.GetConnectionString("DefaultConnection")))
            {
                var sql = "ManagerPatientVisits";
                DynamicParameters param = new DynamicParameters();
                param.Add("PatientId", contractId);
                var result = await conn.QueryAsync<Visit, CareContract, Visit>(
                    sql,
                    (visit, contract) =>
                    {
        ...
        public async Task<List<CareContract>> AssignedPatientContract(int? patientId)
        {
            using (var conn = new SqlConnection(_config.GetConnectionString("DefaultConnection")))
            {
                var sql = "ManagerAssignedPatientContract";
                DynamicParameters param = new DynamicParameters();
                param.Add("PatientId", patientId);
                var result = await conn.QueryAsync<CareContract, EndUser, EndUser, Suburb, Wound, CareContract>(
                    sql,
